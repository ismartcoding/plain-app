...
fun MaterialToolbar.initMenu(@MenuRes menuId: Int, overflow: Boolean = false) {
    menu.clear()
    inflateMenu(menuId)
    if (overflow) {
...
fun MaterialToolbar.updateTitle(vm: BaseItemsModel, rv: RecyclerView, title: Int, total: Int) {
    val bindingAdapter = rv.bindingAdapter
    this.title = if (bindingAdapter.toggleMode) {
...
fun MaterialToolbar.updateFilesTitle(vm: BaseItemsModel, rv: RecyclerView) {
    val bindingAdapter = rv.bindingAdapter
    this.title = if (bindingAdapter.toggleMode) {
        val count = bindingAdapter.getCheckedModels<Any>().size
        LocaleHelper.getStringF(R.string.x_selected, "count", count)
    ...
    this.subtitle = if (bindingAdapter.toggleMode) {
        ""
    } else {
