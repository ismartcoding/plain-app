...
abstract class AbstractMediaController @JvmOverloads constructor(
    ...
    override fun onVideoSizeChanged(mp: IVideoPlayer, width: Int, height: Int) {
        if (width != 0 && height != 0) {
            mRenderView?.setVideoSize(width, height)
        }
    }

    override fun onError(mp: IVideoPlayer, what: Int, extra: Int): Boolean {
        updatePlayerState(false)
        return false
    }

    override fun onInfo(mp: IVideoPlayer, what: Int, extra: Int): Boolean {
        when (what) {
