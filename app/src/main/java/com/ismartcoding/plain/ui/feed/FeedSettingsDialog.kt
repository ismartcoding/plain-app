...
class FeedSettingsDialog : BaseBottomSheetDialog<DialogFeedSettingsBinding>() {
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.autoRefresh.setKeyText(R.string.auto_refresh_feeds)
            .setSwitch(FeedAutoRefreshPreference.get(context), onChanged = { _, isEnabled ->
                lifecycleScope.launch {
                    withIO { FeedAutoRefreshPreference.putAsync(context, isEnabled) }
                    if (isEnabled) {
                        FeedFetchWorker.startRepeatWorker(context)
                    } else {
        ...
        binding.autoRefreshInterval.setKeyText(R.string.auto_refresh_interval)
            .setValueText(FormatHelper.formatSeconds(FeedAutoRefreshIntervalPreference.get(context)))
            .showMore()
            .setClick {
                ...
                SelectOptionsDialog(LocaleHelper.getString(R.string.auto_refresh_interval), options) {
                    lifecycleScope.launch {
                        ...
                        binding.autoRefreshInterval.setValueText(FormatHelper.formatSeconds(FeedAutoRefreshIntervalPreference.get(context)))
                    }
        ...
        binding.autoRefreshOnlyWifi.setKeyText(R.string.auto_refresh_only_over_wifi)
            .setSwitch(FeedAutoRefreshOnlyWifiPreference.get(context), onChanged = { _, isEnabled ->
                lifecycleScope.launch {
                    withIO {
