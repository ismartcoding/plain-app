...
object Patterns {
    ...
    val PY_KEYWORDS = Pattern.compile(
    ...
    val LUA_KEYWORDS = Pattern.compile(
    ...
    val GENERAL_COMMENTS = Pattern.compile(
        "/\\*(?:.|[\\n\\r])*?\\*/|(?<!:)//.*|#.*"
    )

    // same as GENERAL_COMMENTS but without -> //
    val GENERAL_COMMENTS_NO_SLASH = Pattern.compile(
        "/\\*(?:.|[\\n\\r])*?\\*/|#.*"
    )
    val SQL_KEYWORDS = Pattern.compile(
