...
import com.ismartcoding.plain.ui.theme.palette.colorspace.ciexyz.CieXyz.Companion.asXyz
...
data class Oklab(
    ...
    fun toXyz(): CieXyz =
        (
            lmsToXyz *
                (oklabToLms * doubleArrayOf(L, a, b)).map { it.pow(3.0) }
                    .toDoubleArray()
        ).asXyz()

    companion object {
        private val xyzToLms: Matrix3 =
            Matrix3(
        ...
        private val lmsToOklab: Matrix3 =
            Matrix3(
        ...
        fun CieXyz.toOklab(): Oklab = (lmsToOklab * (xyzToLms * xyz).map { it.pow(1.0 / 3.0) }.toDoubleArray()).asOklab()
