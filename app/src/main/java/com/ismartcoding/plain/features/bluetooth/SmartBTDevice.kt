...
class SmartBTDevice(val device: BTDevice) {
    ...
    suspend fun <D : Operation.Data> graphqlAsync(
        ...
        if (r.isSuccess()) {
            try {
                ...
                if (bytes != null) {
                    val adapter = CustomScalarAdapters.Builder()
    ...
    private suspend fun readCharacteristicAsync(device: BTDevice, api: BluetoothApi): BluetoothResult {
        if (device.isConnected()) {
    ...
    private suspend fun writeCharacteristicAsync(device: BTDevice, api: BluetoothApi, value: String): BluetoothResult {
        if (device.isConnected()) {
    ...
    private suspend fun enableNotificationsAsync(device: BTDevice, api: BluetoothApi, enable: Boolean): BluetoothResult {
        if (device.isConnected()) {
    ...
    private suspend fun waitForResultAsync(type: BluetoothActionType, uuid: UUID?, timeout: Long = 2): BluetoothResult {
