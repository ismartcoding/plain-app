...
fun Route.toRouteEdit(): RouteEdit {
    return RouteEdit(
        ...
        Target.parse(target),
        gateway, ifName, isEnabled, notes
    )
}

fun ViewListItemBinding.bindRoute(context: Context, lifecycleScope: LifecycleCoroutineScope, item: Route) {
    clearTextRows()
...
fun Route.getMessage(): String {
    ...
    return LocaleHelper.getStringF(R.string.route_item_description, "target", targetData.getText(UIDataCache.current().getNetworks()), "if_name", ifDisplayName())
}