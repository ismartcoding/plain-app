...
object BluetoothPermission {
    ...
    fun init(activity: AppCompatActivity) {
        enableBluetoothActivityLauncher = activity.registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        ...
        requestBluetoothLocationPermissionLauncher = activity.registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
            if (isGranted) {
                if (BluetoothUtil.isBluetoothReadyToUseWithPermissionRequest()
                ) {
                    sendEvent(BluetoothPermissionResultEvent())
        ...
        requestBluetoothScanConnectPermissionLauncher = activity.registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
        ...
        events.add(receiveEventHandler<RequestEnableBluetoothEvent> {
            enableBluetoothActivityLauncher.launch(Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE))
        })

        events.add(receiveEventHandler<RequestScanConnectBluetoothEvent> {
            requestBluetoothScanConnectPermissionLauncher.launch(arrayOf(Manifest.permission.BLUETOOTH_SCAN, Manifest.permission.BLUETOOTH_CONNECT))
        })

        events.add(receiveEventHandler<RequestBluetoothLocationPermissionEvent> {
            requestBluetoothLocationPermissionLauncher.launch(Manifest.permission.ACCESS_FINE_LOCATION)
        })

        events.add(receiveEventHandler<RequestBluetoothLocationGPSPermissionEvent> {
