...
internal suspend fun BodyTransformedHook.Context.processRange(
    ...
    if (merged.isEmpty()) {
        PARTIAL_CONTENT_LOGGER.trace("Responding 416 RequestedRangeNotSatisfiable for ${call.request.uri}: range is empty")
...
internal fun BodyTransformedHook.Context.processSingleRange(
    ...
    PARTIAL_CONTENT_LOGGER.trace("Responding 206 PartialContent for ${call.request.uri}: single range $range")
