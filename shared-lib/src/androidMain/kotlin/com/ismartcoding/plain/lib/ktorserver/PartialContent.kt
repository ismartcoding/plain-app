...
internal val PARTIAL_CONTENT_LOGGER = KtorSimpleLogger("com.ismartcoding.plain.lib.ktorserver.PartialContent")

/**
 * Vendored from io.ktor:ktor-server-partial-content:3.5.2.
 *
...
public val PartialContent: RouteScopedPlugin<PartialContentConfig> = createRouteScopedPlugin(
    ...
    onCall { call ->
        if (call.request.ranges() == null) {
            PARTIAL_CONTENT_LOGGER.trace("Skip ${call.request.uri}: no ranges specified")
            return@onCall
        }

        if (!call.isGetOrHead()) {
            PARTIAL_CONTENT_LOGGER.trace("Skip ${call.request.uri}: not a GET or HEAD request")
    ...
    on(BodyTransformedHook) { call, message ->
        ...
        if (rangeSpecifier == null) {
            PARTIAL_CONTENT_LOGGER.trace("No range header specified for ${call.request.uri}")
