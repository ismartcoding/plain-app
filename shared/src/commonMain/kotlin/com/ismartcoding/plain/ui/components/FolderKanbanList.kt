...
/**
 * Folder picker options list, shared by the drawer and the bottom-sheet dialog.
 *
 * @param refreshSignal change this value (e.g. a version counter) to force the list to rebuild,
 *        such as after a favorite folder is added/removed elsewhere.
 * @param onNavigateShares when provided, a "Share links" entry is shown at the bottom that
 *        triggers this callback (used by the Files page drawer).
 */
@Composable
fun FolderKanbanList(
    filesVM: FilesViewModel,
    onSelect: (FolderOption) -> Unit,
    refreshSignal: Int = 0,
    onNavigateShares: (() -> Unit)? = null,
) {
        ...
        LazyColumn(Modifier.fillMaxSize()) {
            ...
            items(options) { item ->
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { sendEvent(FolderKanbanSelectEvent(item)); onSelect(item) }
                        .padding(horizontal = 12.dp, vertical = 8.dp),
                    verticalAlignment = Alignment.CenterVertically
                ) {
                    RadioButton(selected = item.isChecked, onClick = { sendEvent(FolderKanbanSelectEvent(item)); onSelect(item) })
            ...
            item { BottomSpace() }
            if (onNavigateShares != null) {
                item {
                    VerticalSpace(dp = 16.dp)
                    PCard {
                        PListItem(
                            modifier = Modifier.clickable { onNavigateShares() },
