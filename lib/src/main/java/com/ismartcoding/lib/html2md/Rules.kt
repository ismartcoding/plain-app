...
internal class Rules {
    ...
    init {
        addRule("blankReplacement", Rule({ element: Node -> ProcessNode.isBlank(element) }) { _: String, node: Node -> if (ProcessNode.isBlock(node)) "\n\n" else "" })
        addRule("paragraph", Rule("p") { content: String, _: Node ->
            "\n\n$content\n\n"
        })
        addRule("br", Rule("br") { _: String, _: Node ->
            "${options.br}\n"
        })
        addRule("heading", Rule(setOf("h1", "h2", "h3", "h4", "h5", "h6")) { content: String, node: Node ->
        ...
        addRule("blockquote", Rule("blockquote") { content: String, _: Node ->
            var c = content.replace("^\n+|\n+$".toRegex(), "")
            c = c.replace("(?m)^".toRegex(), "> ")
            "\n\n$c\n\n"
        })
        addRule("list", Rule(setOf("ul", "ol")) { content: String, node: Node ->
        ...
        addRule("listItem", Rule("li") { content: String, node: Node ->
        ...
        addRule("horizontalRule", Rule("hr") { _: String, _: Node? ->
            "\n\n${options.hr}\n\n"
        })
        addRule(
            "inlineLink",
            Rule({ node: Node -> options.linkStyle == LinkStyle.INLINED && node.nodeName() == "a" && node.attr("href").isNotEmpty() }) { content: String, node: Node ->
        ...
        addRule(
            "referenceLink",
            Rule({ node: Node -> options.linkStyle == LinkStyle.REFERENCED && node.nodeName() == "a" && node.attr("href").isNotEmpty() },
                { content: String, node: Node ->
                    val href = node.attr("href")
                    var title = cleanAttribute(node.attr("title"))
                    if (title.isNotEmpty()) {
        ...
        addRule("emphasis", Rule(setOf("em", "i")) { content: String, _: Node ->
        ...
        addRule("strong", Rule(setOf("strong", "b")) { content: String, _: Node ->
            if (content.isBlank()) {
                ""
            } else {
        ...
        addRule("img", Rule("img") { _: String, node: Node ->
        ...
        addRule("strikethrough", Rule(setOf("del", "s", "strike")) { content: String, _: Node ->
            "~$content~"
        })
        addRule(
            "taskListItems",
        ...
        addRule("default", Rule({ true }) { content: String, element: Node ->
    ...
    private fun addRule(name: String, rule: Rule) {
        rule.name = name
        rules.add(rule)
    }
