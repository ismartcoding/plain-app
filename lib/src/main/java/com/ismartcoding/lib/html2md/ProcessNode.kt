...
internal class ProcessNode {
    ...
    constructor(input: String) {
        val document = Jsoup.parse( // DOM parsers arrange elements in the <head> and <body>.
    ...
    fun flankingWhitespace(): FlankingWhiteSpaces {
        ...
        var trailing = ""
        if (!isBlock(element)) {
            val textContent: String = if (element is Element) {
    ...
    private fun isChildFlankedByWhitespaces(regex: String, sibling: Node?): Boolean {
        if (sibling == null) {
            return false
        }
