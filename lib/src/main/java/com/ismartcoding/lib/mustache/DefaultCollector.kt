 ...
 */
class DefaultCollector @JvmOverloads constructor(private val _allowAccessCoercion: Boolean = true) : BasicCollector() {
    ...
    protected fun getMethod(clazz: Class<*>, name: String): Method? {
    ...
    protected fun getIfaceMethod(clazz: Class<*>?, name: String): Method? {
    ...
    private fun addIfaces(ifaces: MutableSet<Class<*>>, clazz: Class<*>, isIface: Boolean) {
        if (isIface) ifaces.add(clazz)
        for (iface in clazz.interfaces) addIfaces(ifaces, iface, true)
    }

    private fun getMethodOn(clazz: Class<*>, name: String): Method? {
        var m: Method
        try {
    ...
    private fun makeAccessible(m: Method): Method? {
        if (m.isAccessible) return m else if (!_allowAccessCoercion) return null
        m.isAccessible = true
        return m
    }
